...
class MockYumRepositoryLikePlugin(Plugin):
    """
    ...
    def create_configuration(self, plugin_api_context):
        ...
        for node in nodes:
            for package in node.query("yum-repository", is_initial=True):
                tasks.append(
                    ConfigTask(node, package, "Create", "yum-repository",
                               package.name,
                               **self._get_values(package))
                )
            for package in node.query("yum-repository", is_updated=True):
                tasks.append(
                    ConfigTask(node, package, "Update", "yum-repository",
                               package.name,
                               **self._get_values(package))
                )
            for package in node.query("yum-repository", is_for_removal=True):
                tasks.append(
                    ConfigTask(node, package, "deconfigure", "yum-repository",
                               package.name,
