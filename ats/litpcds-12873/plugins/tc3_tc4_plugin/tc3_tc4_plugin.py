...
class Litpcds12873Plugin(Plugin):
    ...
    Required Config task is not returned and also requires 2 other tasks
    ...
    def create_configuration_ms(self, api):
        ...
        for item in ms.routes:
            if item.item_id == 'route_a_tc3' and item.is_initial():
                task4_ms = ConfigTask(ms, item, 'ms routes_a task',
                         'ms::routes_a', 'id_ms_route_a')
                tasks.append(task4_ms)
            if item.item_id == 'route_b_tc3' and item.is_initial():
        ...
        for item in ms.items:
            if item.item_id == 'item_tc3' and item.is_initial():
    ...
    def create_configuration_node(self, api):
        ...
        for node in api.query('node'):
            ...
            for item in node.file_systems:
                if item.item_id == 'file_system_tc4' and item.is_initial():
            ...
            for item in node.configs:
                if item.item_id == 'config_tc4' and item.is_initial():
            ...
            for item in node.items:
                if item.item_id == 'item_tc4' and item.is_initial():
