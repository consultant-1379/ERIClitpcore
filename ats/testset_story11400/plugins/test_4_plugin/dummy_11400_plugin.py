...
class Dummy11400Plugin(Plugin):
    ...
    def create_configuration(self, api):
        ...
        for item in node.query("software-item"):
            tasks.append(CallbackTask(item, 'Item callback task', Dummy11400Plugin._cb))
        for service in node.query("service-base"):
            tasks.append(CallbackTask(service, 'Service callback task', Dummy11400Plugin._cb))
        for config in node.query("node-config"):
            tasks.append(CallbackTask(config, 'Config callback task', Dummy11400Plugin._cb))
        for file_system in node.query("file-system-base"):
            tasks.append(CallbackTask(file_system, 'Filesystem callback task', Dummy11400Plugin._cb))
        for route in node.query("route-base"):
            tasks.append(CallbackTask(route, 'Route callback task', Dummy11400Plugin._cb))
        return tasks
