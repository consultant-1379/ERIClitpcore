        ...
        *An example of package usage:*
        ...
        for node in nodes:
            for package in node.query("mock-package", is_initial=True):
                tasks.append(ConfigTask(node, package, "Description",
                    "package", id=package.name, **self._get_values(package)))
        for node in nodes:
            for package in node.query("mock-package", is_updated=True):
                tasks.append(ConfigTask(node, package, "Update",
                    "package", id=package.name, **self._get_values(package)))
        for node in nodes:
            for package in node.query("mock-package", is_for_removal=True):
                tasks.append(ConfigTask(node, package, "deconfigure",
